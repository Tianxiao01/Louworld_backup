...
const sf::Texture& Characters::getidleTexture()
{
...
bool Characters::IsThisCharacterDoingSth()
{
	return this->thisCharacterIsDoingSth;
}
...
void Characters::idleanimation(float spawnpointX, float spawnpointY)
{
	...
	*/
	...
	this->appearance.setTexture(idleTexture);
...
{	
	...
	//this->thisCharacterIsDoingSth = true;
	...
	else
	{
		...
		//this->thisCharacterIsDoingSth = false;
		this->chosen = false;
	}
}
...

{
	...
	if (this->path.size() >= 2) // the destination is reachable and the decent path is constructed
	{
...
void Characters::hurt(int damage)
